...
class CustomDataset(Dataset):
    def __init__(self, images_file, labels_file, transform=None):
        ...
        """
        self.imgs = torch.load(images_file)
        self.targets = torch.load(labels_file)
    ...
    def __getitem__(self, idx):
        image = transforms.functional.to_pil_image(self.imgs[idx])
        label = self.targets[idx]
        if self.transform:
            image = self.transform(image)
        return image, label
    
def distill_loader(images_file, labels_file, num_workers=2):
    ...
    # /notebooks/Unlearn-Sparse/distilled_data/CIFAR-10_ipc1/images.pt

    # Create data loaders
    custom_train_loader = DataLoader(
        custom_train_set,
        batch_size=len(images_file),
        shuffle=True,
